...
cat("\n\n", strrep("=", 70), "\nCONTRAEJEMPLO: Holt lineal sobre una serie estacionaria sin tendencia\n", strrep("=", 70), "\n", sep = "")

fuente <- fuente_help("lynx")
datos <- leer_serie(lynx, fuente, "numero de linces cazados")
n <- nrow(datos)
y <- datos$y
...
cat("\n1. DESCRIPCION\n")
cat("  Nombre en R: lynx\n  Fuente:", fuente, "\n  Unidad: numero de linces cazados\n")
cat("  Frecuencia:", attr(datos, "frecuencia"), "\n")
cat("  Fechas:", format(min(datos$fecha)), "a", format(max(datos$fecha)), "\n")
cat("  Observaciones:", n, "\n")
...
ggsave("figs/contra-serie.png", graficar_serie(datos, "Linces cazados por año en Canada"), width = 8, height = 4.5)
ggsave("figs/contra-correlograma.png", correlograma(datos), width = 8, height = 6)
...
ggsave("figs/contra-optimizacion.png", grafico_mse(opt, "MSE de un paso - Linces cazados por año en Canada"), width = 7, height = 5)
...
ggsave("figs/contra-final.png",
       grafico_pronostico(datos, n_est, ajuste$yhat, pron, "Linces cazados por año en Canada (holt)", ingenuo = pron_ing),
       width = 8, height = 4.5)
